    ...
    while (client.connected()) {  // loop while the client's connected
      if (client.available()) {   // if there's bytes to read from the
        ...
        if (currentLine.endsWith("GET /H1")) {
          //digitalWrite(LED_BUILTIN, HIGH);  // GET /H1 turns the LED on
          Serial.println("local Turn ON 1");
        }
        if (currentLine.endsWith("GET /L1")) {
          //digitalWrite(LED_BUILTIN, LOW);  // GET /L1 turns the LED off
          Serial.println("local Turn OFF 1");
        }
        if (currentLine.endsWith("GET /L2")) {
          outgoingPacket.message = 20;
          result =
            esp_now_send(broadcastAddress, (uint8_t *)&outgoingPacket,
                         sizeof(outgoingPacket));
          if (result == ESP_OK) {
            Serial.println("Sent with success (Turn OFF 2)");
          } else {
            Serial.println("Error sending the data");
          }
        }
        if (currentLine.endsWith("GET /H2")) {
          outgoingPacket.message = 21;
          result =
            esp_now_send(broadcastAddress, (uint8_t *)&outgoingPacket,
                         sizeof(outgoingPacket));
          if (result == ESP_OK) {
            Serial.println("Sent with success (Turn ON 2)");
          } else {
            Serial.println("Error sending the data");
          }
        }
        if (incomingMessage == 10) {
          Serial.println("recieved command to turn OFF 1");
          incomingMessage = -1;
        } else if (incomingMessage == 11) {
          Serial.println("recieved command to turn ON 1");
          incomingMessage = -1;
        }
      }
    }
